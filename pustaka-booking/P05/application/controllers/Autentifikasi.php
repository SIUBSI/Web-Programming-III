...
class Autentifikasi extends CI_Controller
{
    public function index()
    {
        ...
        $this->form_validation->set_rules('email', 'Alamat Email',
            ...
            'valid_email' => 'Email tidak ditemukan, harap masukkan data dengan benar.'
        ]);
                ...
                if (password_verify($password, $user['password'])) 
                {
                    ...
                    if ($user['role_id'] == 1) {
                        ...
                        if ($user['image'] == 'default.jpg')
                        {
                            ...
                            '<div class="alert alert-info alert-message" role="alert">Silahkan jika ingin ubah foto profil.</div>');
                        }
                    
                        redirect('user');
                    }
                } else {
                    $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert">Password yang anda masukkan tidak benar.</div>');
                    redirect('autentifikasi');
                }

            } else {
                $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert">User tersebut belum diaktivasi.</div>');
                redirect('autentifikasi');
            }
        } else {
            $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert">Email tersebut tidak terdaftar.</div>');
            redirect('autentifikasi');
        }
    }
